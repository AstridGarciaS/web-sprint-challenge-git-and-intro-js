...
function removeArtist(artists, number) {
  let newArtistArray = [...artists];
  newArtistArray.splice(number, 1);
...
function addArtist(artists, name, years, genre, nationality, bio) {
  ...
  let newArtist = {
  name, years, genre, nationality, bio
  };
...
function lotsOfArt(artist) {
  ...
  for (let i = 0; i < artist.length; i++){
    if(artist[i].paintings > 100){
      newArtistArray.push(artist[i].name);
    }
  }
  return newArtistArray;
}
...
function artistInfo(artist, artistsName){
  for(const artist of artists){
    if(artist.name === artistsName){
...
function artistByCountry(artists, nationality){
  ...
  for (const artist of artists){
    if(artist.nationality === nationality){
      artistCountry.push(artist.name);
    }
  }
